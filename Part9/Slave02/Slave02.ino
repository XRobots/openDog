...
// Slave 1 - Front End - top ctrl panel!
...
void setup() {
  ...
  pinMode(33, OUTPUT);              // yellow left bottom
  pinMode(35, OUTPUT);              // tellow right bottom
  pinMode(37, OUTPUT);              // white right top
  pinMode(39, OUTPUT);              // white left top

  // control panel buttons 

  pinMode(25, INPUT_PULLUP);        // right bottom
  pinMode(27, INPUT_PULLUP);        // left bottom
  pinMode(29, INPUT_PULLUP);        // right top
  pinMode(31, INPUT_PULLUP);        // left top  
  ...
  digitalWrite(35, HIGH); // set initial homing LED yellow bottom right
}
   ...
   if (currentMillis - previousMillis >= interval) {  //start timed event
      previousMillis = currentMillis;       
        ...
        if (but1 == 0 && flag == 0) {
          ...
          home4Offset = odrive2.readFloat();
          Serial.println(home4Offset);
          odrive2.SetPosition(0, (home4Offset-(8192*2)));  // back off one revolution
          ...
          flag = 1;
          digitalWrite(33, HIGH);     // Yellow top left
        }      
      
        // ***left leg***
        
        else if (but2 == 0 && flag == 1) {
          digitalWrite(33, LOW);
          ...
          odrive1.SetPosition(0, (home2Offset-(8192*2)));  // back off one revolution 
          ...
          digitalWrite(35, HIGH);   // yellow left
          digitalWrite(33, HIGH);   // yellow right
          flag = 2;  
        }

         else if (but2 == 0 || but1 == 0 && flag == 2) {
           digitalWrite(35, LOW);
           digitalWrite(33, LOW);
         ...
         for (int axis = 0; axis < 2; ++axis) {
           ...
           odrive2.SetPosition(0, (home4Offset - 163840));      // move
           odrive2.SetPosition(1, (home3Offset - 245760));      // move
                         
           odrive1.SetPosition(0, (home2Offset - 163840));      // move
           odrive1.SetPosition(1, (home1Offset - 245760));      // move

           digitalWrite(37, HIGH); // ready for undercarriage calibration
           flag = 3;
        }        
      
        // *****************************leg motor undercarriage**************************************
          
        else if (but3 == 0 && flag == 3) {
          // bend right leg
          digitalWrite(37, LOW);
          ...
          */
          ...
          digitalWrite(39, HIGH);   // white LED left bottom - ready for the other side
          flag = 4;
          }

        //**************** do the other leg *****************
        
        else if (but4 == 0 && flag == 4) {
          digitalWrite(39, LOW);
          ...
          */
          ...
          if (flag == 5) {        // main output to leg starts here
                ...
                if (motorSpeedFlag == 0) {   // only send the data once ever
                        ...
                        Serial2 << "w axis" << 0 << ".controller.config.vel_limit " << 96000.0f << '\n';
                        Serial2 << "w axis" << 1 << ".controller.config.vel_limit " << 96000.0f << '\n';
        
                        Serial3 << "w axis" << 0 << ".controller.config.vel_limit " << 96000.0f << '\n';
                        Serial3 << "w axis" << 1 << ".controller.config.vel_limit " << 96000.0f << '\n';
                ...
                Serial.print(mydata_front.shoulderR);    // print original test value
                Serial.print(" , ");
                shoulderRFiltered = filter(mydata_front.shoulderR, shoulderRFiltered);
                Serial.print(shoulderRFiltered);  
                Serial.print(" , ");  
                Serial.print(mydata_front.shoulderL);    // print original test value
                Serial.print(" , ");
                shoulderLFiltered = filter(mydata_front.shoulderL, shoulderLFiltered);
                Serial.print(shoulderLFiltered);
                Serial.print(" , ");
                
                Serial.print(mydata_front.elbowR);    // print original test value
                Serial.print(" , ");
                elbowRFiltered = filter(mydata_front.elbowR, elbowRFiltered);
                Serial.print(elbowRFiltered);  
                Serial.print(" , ");  
                Serial.print(mydata_front.elbowL);    // print original test value
                Serial.print(" , ");
                elbowLFiltered = filter(mydata_front.elbowL, elbowLFiltered);
                Serial.println(elbowLFiltered);
                ...
                hipRFiltered = filter(mydata_front.hipR, hipRFiltered);    // filtered value in mm
                hipLFiltered = filter(mydata_front.hipL, hipLFiltered);    // filtered value in mm
