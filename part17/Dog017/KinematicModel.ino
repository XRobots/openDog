...
double leg(double Z4, double Y4, double X4, double yaw, double pitch2, double roll, int side, int front) {
           ...
           // ********************** output variables *********************************************                
              ...
              // *********** chunk of code to calculate the motor velovity scaling ********************

              // calcuate the difference in positon between this loop and the last loop

              // *** FRONT RIGHT LEG ***
              mydata_front_diff.hipR = abs(mydata_front_count_corr.hipR - mydata_front_bm.hipR);
              mydata_front_diff.elbowR = abs(mydata_front_count_corr.elbowR - mydata_front_bm.elbowR);
              mydata_front_diff.shoulderR = abs(mydata_front_count_corr.shoulderR - mydata_front_bm.shoulderR);

              // *** FRONT LEFT LEG ***
              mydata_front_diff.hipL = abs(mydata_front_count_corr.hipL - mydata_front_bm.hipL);
              mydata_front_diff.elbowL = abs(mydata_front_count_corr.elbowL - mydata_front_bm.elbowL);
              mydata_front_diff.shoulderL = abs(mydata_front_count_corr.shoulderL - mydata_front_bm.shoulderL);

              // *** BACK RIGHT LEG ***
              mydata_back_diff.hipR = abs(mydata_back_count_corr.hipR - mydata_back_bm.hipR);
              mydata_back_diff.elbowR = abs(mydata_back_count_corr.elbowR - mydata_back_bm.elbowR);
              mydata_back_diff.shoulderR = abs(mydata_back_count_corr.shoulderR - mydata_back_bm.shoulderR);

              // *** BACK LEFT LEG ***
              mydata_back_diff.hipL = abs(mydata_back_count_corr.hipL - mydata_back_bm.hipL);
              mydata_back_diff.elbowL = abs(mydata_back_count_corr.elbowL - mydata_back_bm.elbowL);
              mydata_back_diff.shoulderL = abs(mydata_back_count_corr.shoulderL - mydata_back_bm.shoulderL);

              // scale motor speeds

              // *** FRONT RIGHT LEG ***
              if (mydata_front_diff.hipR > mydata_front_diff.elbowR && mydata_front_diff.hipR > mydata_front_diff.shoulderR) {
                // hip is biggest
              }
              else if (mydata_front_diff.elbowR > mydata_front_diff.hipR && mydata_front_diff.elbowR > mydata_front_diff.shoulderR) {
                // elbow is biggest
              }
              else if (mydata_front_diff.shoulderR > mydata_front_diff.hipR && mydata_front_diff.shoulderR > mydata_front_diff.elbowR) {
                // shoulder is biggest
              }

              // *** FRONT LEFT LEG ***
              if (mydata_front_diff.hipL > mydata_front_diff.elbowL && mydata_front_diff.hipL > mydata_front_diff.shoulderL) {
                // hip is biggest
              }
              else if (mydata_front_diff.elbowL > mydata_front_diff.hipL && mydata_front_diff.elbowL > mydata_front_diff.shoulderL) {
                // elbow is biggest
              }
              else if (mydata_front_diff.shoulderL > mydata_front_diff.hipL && mydata_front_diff.shoulderL > mydata_front_diff.elbowL) {
                // shoulder si biggest
              }

              // *** BACK RIGHT LEG ***
              if (mydata_back_diff.hipR > mydata_back_diff.elbowR && mydata_back_diff.hipR > mydata_back_diff.shoulderR) {
                // hip is biggest
              }
              else if (mydata_back_diff.elbowR > mydata_back_diff.hipR && mydata_back_diff.elbowR > mydata_back_diff.shoulderR) {
                // elbow is biggest
              }
              else if (mydata_back_diff.shoulderR > mydata_back_diff.hipR && mydata_back_diff.shoulderR > mydata_back_diff.elbowR) {
                // shoulder is biggest
              }

              // *** BACK LEFT LEG ***
              if (mydata_back_diff.hipL > mydata_back_diff.elbowL && mydata_back_diff.hipL > mydata_back_diff.shoulderL) {
                // hip is biggest
              }
              else if (mydata_back_diff.elbowL > mydata_back_diff.hipL && mydata_back_diff.elbowL > mydata_back_diff.shoulderL) {
                // elbow is biggest
              }
              else if (mydata_back_diff.shoulderL > mydata_back_diff.hipL && mydata_back_diff.shoulderL > mydata_back_diff.elbowL) {
                // shoulder is biggest
              }      

              // bookmark values from current loop for next time

              mydata_front_bm.hipR = mydata_front_count_corr.hipR;
              mydata_front_bm.hipL = mydata_front_count_corr.hipL;
              mydata_front_bm.shoulderR = mydata_front_count_corr.shoulderR;
              mydata_front_bm.shoulderL = mydata_front_count_corr.shoulderL;
              mydata_front_bm.elbowR = mydata_front_count_corr.elbowR;
              mydata_front_bm.elbowL = mydata_front_count_corr.elbowL;

              mydata_back_bm.hipR = mydata_back_count_corr.hipR;
              mydata_back_bm.hipL = mydata_back_count_corr.hipL;
              mydata_back_bm.shoulderR = mydata_back_count_corr.shoulderR;
              mydata_back_bm.shoulderL = mydata_back_count_corr.shoulderL;
              mydata_back_bm.elbowR = mydata_back_count_corr.elbowR;
              mydata_back_bm.elbowL = mydata_back_count_corr.elbowL;

              // ********** end of motor speed scaling chunk ************************************              
